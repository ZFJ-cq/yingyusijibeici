...
`localStorage` 中。基于遗忘曲线安排复习，卡片式记忆，简洁清爽、响应式、适配电脑打开。

## ✨ 功能特性

| 页面 | 说明 |
| --- | --- |
| **首页看板** | 统计「待复习 / 新词 / 已掌握」数量，总体进度条，两个入口：学新词、复习 |
| **学新词** | 卡片展示单词、音标、释义、例句；【认识 / 不认识】；可自定义每次学习数量 |
| **复习** | 可翻转卡片（正面英文，点击翻面显示释义）；【记得 / 不记得】；结束展示本次统计 |
| **统计** | 学习数据汇总 + 简易图表（状态分布、近 7 天复习活跃度） |
| **设置** | 记录导出备份 / 导入、重置记录、浅色 / 暗黑模式切换、朗读开关与语速 |

**其它**：使用 Web Speech API 实现单词朗读发音。
...
└── src/
    ...
    ├── App.vue                # 布局外壳（顶栏导航 + 主题应用）
    ├── router/
    │   └── index.js           # 路由（hash 模式，静态托管友好）
    ├── data/
    │   └── words.js           # 内置四级单词数据（word / phonetic / meaning / example）
    ├── composables/
    │   ├── useStore.js        # 核心状态管理：遗忘曲线逻辑 + localStorage 持久化 + 统计
...
2. **复习**：到期的单词会出现在复习页，先凭记忆判断，点击卡片可翻面看释义。
   - 【记得】进入下一节点；【不记得】重置进度从头再来；
   - 结束展示本次「记得 / 不记得」统计。
3. **统计**：查看整体进度与近 7 天复习趋势。
4. **设置**：切换主题、调整每次学习数量与朗读、导出/导入备份、重置记录。

## 📚 关于词库数据

`src/data/words.js` 内置了一份经过精选的**四级高频词**演示数据（约 100 词，含音标、释义、例句），
保证项目开箱即用。若需要**完整 2026 版四级词表**，只需将 `WORDS` 数组替换为完整词表，
保持相同字段结构即可，其余代码无需任何改动：
